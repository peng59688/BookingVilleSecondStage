...
import AutoCompelteText  from './AutoCompleteText'
...
export default function RoomFilter({rooms}) {
    ...
    const {
        ...
        minSize,maxSize,breakfast,pets,hotel
    }=context;
...
let hotelName=getUnique(rooms,'hotel');
hotelName=['all',...hotelName]

hotelName=hotelName.map((item,index)=>{
    return <option key={index} value={item}>{item}</option>
})
let hotelItems=getUnique(rooms,'hotel');
hotelItems=[...hotelItems];
    ...
    return (
        ...
        <div className="App-Component">
            <AutoCompelteText items={hotelItems} method={handleChange}/> 
           </div> 
            <form className="filter-form">
            {/* end of hotel search bar */}

             {/* hotel name */}
             <div className="form-group">
                <label htmlFor="type">Hotel name</label>
                <select name="hotel" id="hotel" value={hotel}
